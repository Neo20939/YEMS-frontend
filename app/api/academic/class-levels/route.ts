/**
 * Class Levels API Proxy
 *
 * Proxies get class levels to the backend API
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/academic/class-levels`, {
      method: 'GET',
      headers: {
    ...
    console.error('[Class Levels API] Proxy error:', error)
    return NextResponse.json(
      { error: 'Failed to fetch class levels', message: error.message },
      { status: 500 }
    )
  }
}
