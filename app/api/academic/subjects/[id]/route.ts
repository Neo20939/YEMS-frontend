/**
 * Subject by ID API Proxy
 *
 * Proxies get, update, delete operations for a specific subject
 */
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/academic/subjects/${id}`, {
      method: 'GET',
      headers: {
    ...
    console.error('[Subject API] Proxy error:', error)
    return NextResponse.json(
      { error: 'Failed to fetch subject', message: error.message },
      { status: 500 }
    )
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error('[Subject API] Proxy error:', error)
    return NextResponse.json(
      { error: 'Failed to update subject', message: error.message },
      { status: 500 }
    )
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error('[Subject API] Proxy error:', error)
    return NextResponse.json(
      { error: 'Failed to delete subject', message: error.message },
      { status: 500 }
    )
  }
}
