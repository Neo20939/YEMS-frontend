/**
 * Current Academic Year API Proxy
 *
 * Proxies get current academic year to the backend API
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/academic/academic-years/current`, {
      method: 'GET',
      headers: {
    ...
    console.error('[Current Academic Year API] Proxy error:', error)
    return NextResponse.json(
      { error: 'Failed to fetch current academic year', message: error.message },
      { status: 500 }
    )
  }
}
