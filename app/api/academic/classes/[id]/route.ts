/**
 * Class by ID API Proxy
 *
 * Proxies get, update, delete operations for a specific class
 */
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/academic/classes/${id}`, {
      method: 'GET',
      headers: {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch class', message: error.message },
      { status: 500 }
    )
  }
}
