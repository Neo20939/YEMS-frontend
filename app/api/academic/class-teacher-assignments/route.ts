/**
 * Class Teacher Assignments API Proxy
 *
 * Proxies class teacher assignment operations to the backend API
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[Class Teacher Assignments API] Proxy error:', error)
    return NextResponse.json(
      { error: 'Failed to fetch class teacher assignments', message: error.message },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[Class Teacher Assignments API] Proxy error:', error)
    return NextResponse.json(
      { error: 'Failed to create class teacher assignment', message: error.message },
      { status: 500 }
    )
  }
}
