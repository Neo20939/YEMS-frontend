/**
 * Set Current Term API Proxy
 *
 * Proxies setting current term to the backend API
 */
...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/academic/terms/${id}/set-current`, {
      method: 'POST',
      headers: {
    ...
    console.error('[Set Current Term API] Proxy error:', error)
    return NextResponse.json(
      { error: 'Failed to set current term', message: error.message },
      { status: 500 }
    )
  }
}
