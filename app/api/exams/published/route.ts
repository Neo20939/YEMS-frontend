...
export async function GET(request: NextRequest) {
  ...
  const url = `${API_BASE_URL}/api/exams/published${queryString ? `?${queryString}` : ''}`
  ...
  try {
    ...
    return NextResponse.json({ message: 'Failed to fetch published exams', error: error.message }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    const axios = require('axios')
    const response = await axios.default.post(`${API_BASE_URL}/api/exams/published`, body, {
      headers: {
        'Cookie': sessionCookie ? `yems_session=${sessionCookie}` : '',
