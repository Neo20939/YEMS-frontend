...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await axios.default.post(`${API_BASE_URL}/api/exams/submissions`, body, {
      headers: {
    ...
    return NextResponse.json({ message: 'Failed to start exam', error: error.message }, { status: 500 })
  }
}