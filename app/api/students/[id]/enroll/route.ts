...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }>}) {
  ...
  try {
    ...
    const response = await axios.default.post(`${API_BASE_URL}/api/students/${id}/enroll`, body, {
      headers: {
    ...
    return NextResponse.json({ message: 'Failed to enroll student', error: error.message }, { status: 500 })
  }
}