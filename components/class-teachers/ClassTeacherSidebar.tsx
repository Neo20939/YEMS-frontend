...
const navItems = [
  { name: "Dashboard", href: "/class-teachers", icon: BarChart3 },
  { name: "My Classes", href: "/class-teachers/classes", icon: GraduationCap },
  { name: "Student Records", href: "/class-teachers/students", icon: Users },
  { name: "Academic Reports", href: "/class-teachers/reports", icon: ClipboardList },
]

export default function ClassTeacherSidebar({
  teacherName: propName,
  teacherId: propId,
}: ClassTeacherSidebarProps) {
  const { user } = useUser()
  const teacherName = user?.name || propName || "Class Teacher"
  const teacherId = user?.id || propId || "CT001"
